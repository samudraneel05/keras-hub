...
# 2. AutoencoderKLQwenImage VAE for image encoding/decoding
# 3. Complete text encoder integration
# 4. Flow matching scheduler implementation
# This currently only implements the diffusion transformer backbone with
# condition injection.
...
class QwenImageEditTimestepEmbedding(layers.Layer):
...
class QwenImageEditAttention(layers.Layer):
    ...
    def call(
        self, hidden_states, encoder_hidden_states=None, condition_images=None
    ):
        ...
        if condition_images is not None:
            cond_key = self.k_proj(condition_images)
            cond_value = self.v_proj(condition_images)
            key = ops.concatenate([key, cond_key], axis=1)
            value = ops.concatenate([value, cond_value], axis=1)
...
class QwenImageEditTransformerBlock(layers.Layer):
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, **kwargs):
        ...
        self.attn = QwenImageEditAttention(hidden_size, num_heads)
    ...
    def call(
        ...
        hidden_states = self.attn(
            hidden_states, encoder_hidden_states, condition_images
        )
...
@keras_hub_export("keras_hub.models.QwenImageEditBackbone")
class QwenImageEditBackbone(Backbone):
    """Qwen-Image-Edit backbone for image-to-image editing.

    This model implements the Qwen-Image-Edit architecture for advanced
    image editing tasks. It supports multi-image inputs, character consistency,
    and various editing operations with strong geometric reasoning.
    ...
    Args:
        ...
        image_size: int. The size of input/output images.
    ...
    Example:
    ```python
    backbone = keras_hub.models.QwenImageEditBackbone(
        hidden_size=3072,
        num_layers=24,
        num_heads=24,
        image_size=1024,
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.timestep_embedding = QwenImageEditTimestepEmbedding(hidden_size)
        self.text_projection = layers.Dense(hidden_size)
        self.condition_projection = layers.Dense(hidden_size)

        self.patch_embedding = layers.Dense(hidden_size)

        self.transformer_blocks = [
            QwenImageEditTransformerBlock(hidden_size, num_heads, mlp_ratio)
        ...
        condition_latents = self.condition_projection(condition_latents)
        ...
        for block in self.transformer_blocks:
            hidden_states = block(
                ...
                condition_images=condition_latents,
            )
